  ...
  const handleBookNow = () => {
    const bookingData = {
      room: {
        ...
        roomImagePaths: room.roomImagePaths,
      }
    };

    // Use new enhanced booking flow
    navigate('/booking/enhanced', { state: bookingData });
  };
    ...
    <div className="max-w-sm rounded-2xl border border-slate-200 bg-white p-6 shadow-sm sticky top-24">
      ...
      <div className="mb-5 space-y-2">
        ...
        {room.quantity && (
          <div className="flex items-center justify-between text-sm">
            ...
            <span className="font-medium">{room.quantity} rooms</span>
          </div>
        )}
      </div>
      ...
      <div className="mt-4 pt-4 border-t border-slate-200">
        <p className="text-xs text-slate-500 text-center">
          ✨ New enhanced booking experience with add-ons
        </p>
      </div>
    </div>
  );
